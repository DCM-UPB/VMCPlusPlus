...
int main()
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (i > 0) {
            ...
            if (isOdd[i]) {
                assert(fabs(protov_asym[0] + protov_asym[1]) < SUPERTINY);
            }
    ...
    for (WaveFunction * wf : wfs) {
        ...
        double samp;
        wf->protoFunction(x, &samp);
        f = wf->computeWFValue(&samp);
        ...
        for (int i = 0; i < NTOTALDIM; ++i) {
            for (int j = 0; j < wf->getNVP(); ++j) {
                ...
                x[i] = origx - DX;
                wf->protoFunction(x, &samp);
                fmdxdvp = wf->computeWFValue(&samp);
