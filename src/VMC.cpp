...
VMC::VMC(std::unique_ptr<WaveFunction> wf, std::unique_ptr<Hamiltonian> H, const int nskip_eg):
        _mci(H->getTotalNDim()), _wf(wf.get()/*remains valid (until destruct)*/), _H(H.get()), _nskip_eg(nskip_eg)
{
    ...
    _mci.addObservable(std::move(H), 1, _nskip_eg);
}

VMC::VMC(const WaveFunction &wf, const Hamiltonian &H, const int nskip_eg):
        _mci(H.getTotalNDim()),
        ...
        _nskip_eg(nskip_eg)
{
    ...
    _mci.addObservable(std::unique_ptr<mci::ObservableFunctionInterface>(_H), 1, _nskip_eg);
}
