...
nfm::NoisyValue EnergyGradientTargetFunction::f(const std::vector<double> &vp)
{
    ...
    if (_lambda_reg > 0.) { // compute the regularization term
        ...
        f.value += _lambda_reg*norm/_wf->getNVP();
    }
...
void EnergyGradientTargetFunction::grad(const std::vector<double> &vp, std::vector<nfm::NoisyValue> &grad)
{
    fgrad(vp, grad);
}


nfm::NoisyValue EnergyGradientTargetFunction::fgrad(const std::vector<double> &vp, std::vector<nfm::NoisyValue> &grad)
{
    // set the variational parameters given as input
    _wf->setVP(vp.data());
    ...
    for (int i = 0; i < _wf->getNVP(); ++i) {
        grad[i].value = 2.*(HOi[i] - H[0]*Oi[i]);
        grad[i].error = 2.*(dHOi[i] + fabs(H[0]*Oi[i])*(dH[0]/H[0] + dOi[i]/Oi[i]));
    }
    ...
    if (_lambda_reg > 0.) { // compute the regularization terms
        ...
        f.value += fac*norm;
        for (int i = 0; i < _wf->getNVP(); ++i) { grad[i].value += 2.*fac*vp[i]; }
    }
