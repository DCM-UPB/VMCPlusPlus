...
void TwoBodyPseudoPotential::d1(const double * r1, const double * r2, double * deriv1){
    ...
    _dist->distD1(r2, r1, deriv1+_dist->getNSpaceDim());
...
void TwoBodyPseudoPotential::d2(const double * r1, const double * r2, double * deriv2){
    ...
    for (int i=0; i<_dist->getNSpaceDim(); ++i){
        deriv2[i] *= ud1;
    }
    _dist->distD1(r1, r2, _foo);
    for (int i=0; i<_dist->getNSpaceDim(); ++i){
        deriv2[i] += _foo[i] * _foo[i] * ud2;
    }


    _dist->distD2(r2, r1, deriv2+_dist->getNSpaceDim());
    for (int i=0; i<_dist->getNSpaceDim(); ++i){
        deriv2[_dist->getNSpaceDim()+i] *= ud1;
    }
    _dist->distD1(r2, r1, _foo);
    for (int i=0; i<_dist->getNSpaceDim(); ++i){
        deriv2[_dist->getNSpaceDim()+i] += _foo[i] * _foo[i] * ud2;
    }
}
