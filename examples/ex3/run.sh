...
echo "$CC $FLAGS $DEBUGFLAGS $IMCI $INFM $IFFNN -I${ROOT_FOLDER}/src/ -I/usr/local/include -c *.cpp"
...
case ${OS_NAME} in
   "Darwin")
      echo "$CC $FLAGS $DEBUGFLAGS -L${ROOT_FOLDER} $LMCI $LNFM $LFFNN $LGSL -o exe *.o -l$LIBNAME $LIBMCI $LIBNFM $LIBGSL $LIBFFNN"
      $CC $FLAGS $DEBUGFLAGS -L${ROOT_FOLDER} $LMCI $LNFM $LFFNN $LGSL -o exe *.o -l$LIBNAME $LIBMCI $LIBNFM $LIBFFNN $LIBGSL
