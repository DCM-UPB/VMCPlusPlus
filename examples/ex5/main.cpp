...
int main(){
   ...
   const int N_HID_LY = 1;
   const int N_HID_UNITS = 14;
   NeuralWaveFunction * psi = new NeuralWaveFunction(N_HID_LY, N_HID_UNITS);
   ...
   const long G_NMC = 5000l; // MC samplings to use for computing the energy gradient
   double * energy = new double[4]; // energy
   double * d_energy = new double[4]; // energy error bar
   double * vp = new double[psi->getNVP()];
      
   
   cout << "-> ham:    w = " << w1 << endl << endl;
   cout << "-> psi:    N_HID_LY = " << N_HID_LY << "     N_HID_UNITS = " << N_HID_UNITS << endl << endl;
   ...
   writePlotFile(psi->getFFNN(), base_input, input_i, output_i, min, max, npoints, "getOutput", "wf_begin.txt");
   ...
   //vmc->dynamicDescentOptimization();
   vmc->simulatedAnnealingOptimization();
   cout << "   . . . Done!" << endl << endl;
   ...
   writePlotFile(psi->getFFNN(), base_input, input_i, output_i, min, max, npoints, "getOutput", "wf_opt.txt");
